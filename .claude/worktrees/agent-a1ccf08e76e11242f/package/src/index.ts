...
import { generateLumineFile, generateSharedModule } from "./emit";
import { detectCycleGroups } from "./cycle";
...
async function run(ctx: RunContext = {}) {
    ...
    const luauFiles = walkLuau(outDir).filter((f) =>
        f !== lumineFilePath && !basename(f).startsWith("_lumine_shared_"),
    );
    ...
    // ── Phase 0: Resolve circular type dependencies ───────────────────────────
    const cycleGroups = detectCycleGroups(manifests, globalTypeOrigins, cwd);
    const liftedTypeNames = new Set<string>();
    for (const group of cycleGroups) {
        const content = generateSharedModule(group, globalTypeOrigins, rojoProject, cwd);
        const contentHash = hashString(content);
        if (contentHash !== hashFile(group.sharedFilePath)) {
            if (!dryRun) {
                mkdirSync(dirname(group.sharedFilePath), { recursive: true });
                writeFileSync(group.sharedFilePath, content, "utf-8");
            }
            console.log(`[lumine] wrote shared types module: ${relative(cwd, group.sharedFilePath)}`);
        }
        for (const name of group.liftedTypeNames) {
            globalTypeOrigins.set(name, group.sharedFilePath);
            liftedTypeNames.add(name);
        }
    }
    ...
    for (const luauPath of luauFiles) {
        ...
        if (
            cached &&
            cached.contentHash === srcHash &&
            cached.dtsHash === dtsHash &&
            dtsChangedCount === 0
        ) {
